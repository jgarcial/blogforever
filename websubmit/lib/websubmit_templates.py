        ...
        out = """
                <div class="span6 offset3">
                        ...
                        <ul class="nav nav-list"> 
                            <li><a href="#"></i> <h4>%(documents)s<br/></h4></a></li>
                        </ul>
                        </div>
                </div>""" % {'documents': catalogues}
        return out
...
#                    out = "%s\n" % cgi.escape(catalog['name'])
    ...
    def tmpl_action_page(self, ln, uid, pid, now, doctype,
        ...
        for i in range(0, len(actionShortDesc)):
            out += """<input type="submit" class="adminbutton" value="%(status)s" onclick="if (tester()) { document.forms[0].indir.value='%(indir)s';document.forms[0].act.value='%(act)s';document.forms[0].submit();}; return false;" /><br /><br />""" % {
                     'status' : statustext[i],
                     'indir' : indir[i],
    ...
    def tmpl_page_interface(self, ln, docname, actname, curpage, nbpages, nextPg, access, nbPg, doctype, act, fields, javascript, mainmenu):
        ...
        # top menu
        out = """
                <div class="span6 offset2"><form class="form-horizontal well" method="post" action="/submit" enctype="multipart/form-data" onsubmit="return tester();" accept-charset="UTF-8">
              """ % {'docname' : docname, 'actname' : actname,}
        ...
        if int(curpage) != 1:
            ...
            out += """ &nbsp;"""
        out += """</form>
                 <a href="%(mainmenu)s" onclick="if (%(check_not_already_enabled)s){return confirm('%(surequit)s')}"></a>"""
