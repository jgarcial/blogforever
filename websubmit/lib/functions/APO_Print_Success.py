...
def APO_Print_Success(parameters, curdir, form, user_info=None):
    ...
    if decision != "":
        if post_title:
            ...
            id = post_url
        if decision == "approve":
            additional_info = "The post record <b>%s</b> will be deleted from " \
                                "the repository ." % cgi.escape(id)
        else:
            additional_info =  "The post record <b>%s</b> will not be deleted from " \
                                      "the repository ."  % cgi.escape(id)
