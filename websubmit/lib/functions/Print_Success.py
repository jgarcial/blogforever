...
def Print_Success(parameters, curdir, form, user_info=None):
    ...
    t=t+Request_Print("A",  "Your blog has the following reference(s): <b>%s%s</b><br /><br />" % (rn,additional_rn))
    if status == "APPROVAL":
        t=t+Request_Print("A",  "An email has been sent to the referee. You will be warned by email as soon as the referee takes his/her decision regarding your blog.<br /><br />\n")
    if status == "ADDED":
        t=t+Request_Print("A",  "It will soon available on our server.<br /><br />\n")
