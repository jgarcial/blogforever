...
CFG_MAIL_BODY = """
A request for the approval of the deletion of a post record in the %(site_name)s has been
made and requires your attention as a referee.  The details of the post record are as follows:

   Title: %(title)s
   Author: %(author)s
   Post URL: [%(post_url)s]

You can see the details of the post record at the following address:
...
def DPI_Mail_Approval_Request_to_Referee(parameters, curdir, form, user_info=None):
    """
    This function sends an email to the referee in order to start the
    approval process related to the deletion of a post record.
    ...
    post_author = "".join(["%s" % author.strip() for author in \
                          get_fieldvalues(int(sysno), "100__a")])
    ...
    # Send the email:
    mail_subject = "Request for post deletion approval : [%(id)s]"
    if post_title:
        mail_subject = mail_subject % {'id': post_title}
    else:
        mail_subject = mail_subject % {'id': post_url}
