...
def APM_Mail_Final_Decision_to_User (parameters, curdir, form, user_info=None):
    """
    ...
    if comments_filename in (None, "", "NULL"):
        ...
        if os.path.exists("%s/COM" % curdir):
            try:
                ...
                exception_prefix = "Error in WebSubmit function " \
                                   "APS_Mail_Final_Decision_to_User. Tried to open " \
        ...
        if os.path.exists("%s/%s" % (curdir, comments_filename)):
            try:
                ...
                comment = ""
                exception_prefix = "Error in WebSubmit function " \
                                "APS_Mail_Final_Decision_to_User. Tried to open comments " \
    ...
    if decision_filename in (None, "", "NULL"):
        ...
        try:
            ...
            decision = ""
            exception_prefix = "Error in WebSubmit function " \
                               "APS_Mail_Final_Decision_to_User. Tried to open decision " \
                               "file [%s/%s] but was unable to." \
    ...
    if decision != "":
        if decision == "approve":
            mailtitle = "Blog record modification approved: [%(id)s]"
            if blog_title:
                mailtitle = mailtitle % {'id': blog_title}
            else:
                mailtitle = mailtitle % {'id': blog_url}
            mailbody = "\nModifications done on the metadata of the blog record with URL [%s] and title '%s' have been approved.\n" % (blog_url, blog_title)
            mailbody += "\nIt will be soon accessible here: <%s/%s/%s>\n" % (CFG_SITE_URL,CFG_SITE_RECORD,sysno)
        else:
            mailtitle = "Blog record modification rejected: [%(id)s]"
            if blog_title:
                mailtitle = mailtitle % {'id': blog_title}
            else:
                mailtitle = mailtitle % {'id': blog_url}
            mailbody = "\nModifications done on the metadata of the blog record with URL [%s] and title '%s' have been rejected.\n" % (blog_url, blog_title)
    if comment != "":
