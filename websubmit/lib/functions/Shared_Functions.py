...
def get_nice_bibsched_related_message(curdir, ln=CFG_SITE_LANG):
    ...
    if server_pid():
        ...
        msg = _("""The task queue is currently running in automatic mode and there are currently %s tasks waiting to be executed. \n
Your record should be available within a few minutes and searchable within an hour or thereabouts.\n""") % (len(res))
    else:
        msg = _("""Because of a human intervention or a temporary problem, the task queue is currently set to the manual mode. \n
                Your submission is well registered but may take longer than usual before it is fully integrated and searchable.\n""")
