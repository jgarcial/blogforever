...
def APP_Print_Success(parameters, curdir, form, user_info=None):
    ...
    if decision != "":
        if blog_title:
            ...
            id = blog_url
        if decision == "approve":
            additional_info = "The blog record <b>%s</b> and all its comments and posts will be deleted from " \
                                "the repository ." % cgi.escape(id)
        else:
            additional_info =  "The blog record <b>%s</b> will not be deleted from " \
                                      "the repository ."  % cgi.escape(id)
