...
def APM_Print_Success(parameters, curdir, form, user_info=None):
    ...
    if decision_filename in (None, "", "NULL"):
        ...
        if os.path.exists("%s/decision" % curdir):
            try:
                ...
                exception_prefix = "Error in WebSubmit function " \
                                   "APM_Print_Success. Tried to open " \
        ...
        try:
            ...
            decision = ""
            exception_prefix = "Error in WebSubmit function " \
                               "APM_Print_Success. Tried to open decision " \
    ...
    if decision != "":
        if blog_title:
            ...
            id = blog_url
        if decision == "approve":
            additional_info = "The blog record <b>%s</b> will be updated into " \
                                "the repository ." % cgi.escape(id)
        else:
            additional_info =  "The blog record <b>%s</b> will not be updated into " \
                                      "the repository ."  % cgi.escape(id)
