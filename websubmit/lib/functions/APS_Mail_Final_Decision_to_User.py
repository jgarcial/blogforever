...
def APS_Mail_Final_Decision_to_User (parameters, curdir, form, user_info=None):
    """
    ...
    if decision != "":
        if decision == "approve":
            mailtitle = "Blog record submission approved: [%s]" % rn
            mailbody = "\nThe blog record with reference number [%s] has been approved.\n" % rn
            mailbody += "It will be soon accessible here: <%s/%s/%s>\n" % (CFG_SITE_URL,CFG_SITE_RECORD,sysno)
        else:
            mailtitle = "Blog record submission rejected: [%s]" % rn
            mailbody = "\nThe blog record with reference number [%s] has been rejected.\n" % rn
    if comment != "":
