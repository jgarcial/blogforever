  ...
  {'CFG_SITE_URL': CFG_SITE_URL,
   ...
   'html_search_syntax': self.tmpl_opensearch_html_url_syntax
    ...
    def tmpl_record_body(self, titles, authors, dates, rns, abstracts, urls_u, urls_z, ln):
        ...
        out = ""
        for title in titles:
            out += "<strong>%(title)s</strong> TEST" % {
...
#        out = """<data wiki-url = "" wiki-section = "">\n"""
        out = """<data>\n"""
        return out
...
#        banner = self.tmpl_print_searchresultbox("<strong>" + _("Frequent co-authors:") + "</strong>", collabstr)
    ...
    def tmpl_sru_diagnostics_prologue(self, params):
        """ Specification: http://www.loc.gov/standards/sru/specs/common.html
            The diagnostics xml document is presented in case of error"""
        out = """<?xml version="1.0"?>
                <zs:diagnostics xmlns:diag="http://www.loc.gov/zing/srw/diagnostic/" xmlns:zs="http://www.loc.gov/zing/srw/">
                <diag:diagnostic>
                    <diag:uri>info:srw/diagnostic</diag:uri>
                    <diag:message>%s</diag:message>
                    <diag:details>%s</diag:details>""" % (params['message'], params['details'])
        return out

    def tmpl_sru_diagnostics_epilogue(self):
        """ End of diagnostics error message"""
        out = """     </diag:diagnostic>
                </zs:diagnostics>"""
        return out
