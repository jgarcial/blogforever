...
"""
Collector for vote history of the user
...
class VoteHistory(HistoryCollector):
    """
    Collects vote history
    """
    icon = 'thumbs-up'
    label = 'Votes'
    ...
    def get_user_history(self, limit, offset, filter_from, filter_to):
        ...
        votes = (db.session
                 ...
                 .filter(CmtACTIONHISTORY.action_code == 'V')
                 .filter(CmtACTIONHISTORY.id_cmtRECORDCOMMENT
                         == CmtRECORDCOMMENT.id))

        if filter_from:
            votes = votes.filter(CmtACTIONHISTORY.action_time >= filter_from)

        if filter_to:
            votes = votes.filter(CmtACTIONHISTORY.action_time <= filter_to)
    ...
    def get_message(entry):
        """
        Returns a message to display vote history on web-interface.
        ...
        if entry.star_score == 0:
            ...
            return _(CFG_WEBHISTORY_MSGS[9]) % {'recordurl': recordurl,
                                                'title': title,
                                                'discussionurl': discussionurl}

        else:
            reviewurl = url_for('webcomment.reviews', recid=entry.id_bibrec)
            return _(CFG_WEBHISTORY_MSGS[10]) % {'recordurl': recordurl,
                                                 'title': title,
                                                 'reviewurl': reviewurl}
    ...
    def get_share_message(serialized_history_element):
        """
        Returns a message used while sharing vote history on
        web-interface.
        ...
        if int(serialized_history_element[2]) == 0:
            ...
            return _(CFG_SHARE_MSGS[9]) % {'recordurl': recordurl,
                                           'title': title,
                                           'discussionurl': discussionurl}
            ...
            reviewurl = url_for('webcomment.reviews',
                                recid=serialized_history_element[1])
            return _(CFG_SHARE_MSGS[10]) % {'recordurl': recordurl,
                                            'title': title,
                                            'reviewurl': reviewurl}
COLLECTOR = VoteHistory
